...
describe('Prisoner search service', () => {
  ...
  describe('getPrisoners', () => {
    describe('prisoner search', () => {
      ...
      it('Propagates error', async () => {
        ...
        await expect(prisonerSearchService.getPrisoners(search, prisonId, username, 0)).rejects.toEqual(
  ...
  describe('getPrisoner', () => {
    it('should return null if no matching prisoner', async () => {
      ...
      const result = await prisonerSearchService.getPrisoner('test', prisonId, username)
      expect(result).toBe(null)
    })

    it('should return matching prisoner', async () => {
      prisonerSearchClient.getPrisoner.mockResolvedValue({ content: [prisoner] })
      const result = await prisonerSearchService.getPrisoner('A1234BC', prisonId, username)
  ...
  describe('getPrisonerById', () => {
    it('should return prisoner details for given prisoner ID', async () => {
      ...
      const result = await prisonerSearchService.getPrisonerById('A1234BC', username)
  ...
  describe('getPrisonerNotFoundMessage', () => {
    it('should handle prisoner not found in any establishment', async () => {
      ...
      const message = await prisonerSearchService.getPrisonerNotFoundMessage('A1234BC', prisonName, username)
    ...
    it('should propagate errors other than NotFound (404) ', async () => {
      ...
      await expect(
        prisonerSearchService.getPrisonerNotFoundMessage('A1234BC', prisonName, username),
